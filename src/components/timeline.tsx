...
export function Timeline() {
  return (
    <Section id="timeline" className="bg-blue-gray-50/60">
      <SectionHeading
        kicker="Background"
        ...
        {TIMELINE.map((n, i) => (
          <li key={n.org} className="flex gap-5">
            <div className="flex flex-col items-center">
              <span className="w-3.5 h-3.5 rounded-full bg-blue-gray-900 mt-1.5 shrink-0" />
              {i < TIMELINE.length - 1 && <span className="w-px flex-1 bg-blue-gray-200 my-1" />}
            </div>
            <div className="pb-10 max-w-3xl">
              <p className="text-xs font-bold uppercase tracking-widest text-blue-gray-400 mb-1">{n.period}</p>
              <h3 className="text-blue-gray-900 font-bold text-lg leading-snug">
                {n.url ? (
              ...
              <p className="text-blue-gray-500 text-sm mb-2">{n.role}</p>
              <p className="text-blue-gray-600 text-sm leading-relaxed mb-3">{n.blurb}</p>
